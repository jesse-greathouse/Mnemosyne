...
env.update(
    {
        ...
        "CELERY_WORKER_CONCURRENCY": cnf["celery"].get(
            "CELERY_WORKER_CONCURRENCY", "2"
        ),
        # Redis (optional, but likely used somewhere)
        "REDIS_HOST": cnf.get("redis", {}).get(
...
def start_web():
    if SUPERVISOR_PID.exists():
        ...
        if pid and pid.isdigit() and is_pid_running(pid):
            ...
            subprocess.run(
                ["supervisorctl", "-c", str(SUPERVISOR_CONF), "start", "all"], env=env
            )
            return
...
def stop_web():
    if SUPERVISOR_PID.exists() and is_pid_running(SUPERVISOR_PID.read_text().strip()):
        ...
        subprocess.run(
            ["supervisorctl", "-c", str(SUPERVISOR_CONF), "stop", "all"], env=env
        )
    else:
...
def restart_web():
    if SUPERVISOR_PID.exists() and is_pid_running(SUPERVISOR_PID.read_text().strip()):
        ...
        subprocess.run(
            ["supervisorctl", "-c", str(SUPERVISOR_CONF), "restart", "all"], env=env
        )
    else:
        print("No running supervisor daemon found.")
