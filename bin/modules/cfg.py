...
CONFIG_FILE_PATH = APP_ROOT / ".mnemosyne-cfg.yml"
ENV_FILE_PATH = APP_ROOT / ".env"


def get_config_file():
    """
    Determine the path to the mnemosyne YAML configuration file.
...
def get_configuration():
    """
    Load the mnemosyne configuration from YAML, creating a default config if needed.
...
def _substitute_env_placeholders(config_fragment, env_dict):
    ...
    if isinstance(config_fragment, dict):
        return {
            key: _substitute_env_placeholders(val, env_dict)
            for key, val in config_fragment.items()
        }
    elif isinstance(config_fragment, list):
        return [
            _substitute_env_placeholders(item, env_dict) for item in config_fragment
        ]
