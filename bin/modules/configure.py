...
CONFIG_FILE = APP_ROOT / ".mnemosyne-cfg.yml"

# Config templates and their output files
CONFIG_FILES = {
    "celery": [
        ETC_DIR / "supervisor/celery.conf.dist",
        ETC_DIR / "supervisor/celery.conf",
    ],
    "nginx": [ETC_DIR / "nginx/nginx.dist.conf", ETC_DIR / "nginx/nginx.conf"],
    "supervisord": [
        ETC_DIR / "supervisor/conf.d/supervisord.conf.dist",
        ETC_DIR / "supervisor/conf.d/supervisord.conf",
    ],
    "force_ssl": [
        ETC_DIR / "nginx/force-ssl.dist.conf",
        ETC_DIR / "nginx/force-ssl.conf",
    ],
    "ssl_params": [
        ETC_DIR / "nginx/ssl-params.dist.conf",
        ETC_DIR / "nginx/ssl-params.conf",
    ],
    "queue_manager": [
        ETC_DIR / "supervisor/queue-manager.conf.dist",
        ETC_DIR / "supervisor/queue-manager.conf",
    ],
}
...
def configure(interactive: bool = True):
    ...
    env_values = {}
    env_values.update(merged.get("django", {}))
    env_values.update(merged.get("celery", {}))
    env_values.update(merged.get("rabbitmq", {}))
    env_values.update(merged.get("redis", {}))
    cfg.write_env_file(ENV_FILE, env_values)
...
def interactive_prompt(cnf: dict) -> dict:
    ...
    prompts = [
        ...
        ("DATABASE_NAME", "Database Name", "mnemosyne"),
        ("DATABASE_USER", "Database User", "mnemosyne"),
        ("DATABASE_PASSWORD", "Database Password", "mnemosyne"),
    ...
    for key, label, default in prompts:
        ...
        display_default = (
            "*****" if is_password and (existing or default) else existing or default
        )
...
"""
    ...
    if choice in ("y", "yes"):
        ...
        choice = (
            input("Update superuser instead? (y or n) [default n] ").strip().lower()
        )
        if choice in ("y", "yes"):
...
def assign_dynamic_config(cnf: dict) -> dict:
    ...
    if is_ssl:
        ...
        cnf["nginx"][
            "SSL_CERT_LINE"
        ] = f"ssl_certificate {cnf['nginx'].get('SSL_CERT', ETC_DIR / 'ssl/certs/mnemosyne.cert')}"
        cnf["nginx"][
            "SSL_KEY_LINE"
        ] = f"ssl_certificate_key {cnf['nginx'].get('SSL_KEY', ETC_DIR / 'ssl/private/mnemosyne.key')}"
        cnf["nginx"][
            "INCLUDE_FORCE_SSL_LINE"
        ] = f"include {ETC_DIR}/nginx/force-ssl.conf"
    else:
        cnf["nginx"]["SSL"] = ""
    ...
    rabbitmq_host = cnf["rabbitmq"].get("RABBITMQ_HOST", "localhost")
    rabbitmq_port = cnf["rabbitmq"].get("RABBITMQ_PORT", "5672")
    rabbitmq_vhost = cnf["rabbitmq"].get("RABBITMQ_VHOST", "mnemosyne")
    rabbitmq_user = cnf["rabbitmq"].get("RABBITMQ_USER", "mnemosyne")
    rabbitmq_password = cnf["rabbitmq"].get("RABBITMQ_PASSWORD", "mnemosyne")
    ...
    csrf_origins = [
        f"http://{h}:{port}" if not is_default_port else f"http://{h}"
        for h in parsed_hosts
    ] + [
        f"https://{h}:{port}" if not is_default_port else f"https://{h}"
        for h in parsed_hosts
    ]
    ...
    supervisor_secret = cnf["supervisord"].get(
        "SUPERVISORCTL_SECRET"
    ) or utility.generate_rand_str(32)
    queue_secret = cnf["queue_manager"].get(
        "QUEUECTL_SECRET"
    ) or utility.generate_rand_str(32)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--non-interactive", action="store_true", help="Run without prompting for input"
    )
